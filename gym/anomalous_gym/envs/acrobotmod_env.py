...
class ModAcrobotEnv(core.Env):
    ...
    **REFERENCE:**
    ...
    def __init__(self, case, when_anomaly_starts=None):
        self.viewer = None
        ...
        if self._clock > self.when_anomaly_starts:
            for sensor_i, sensor in enumerate(self.selected_sensors):
                if sensor:
                    # Add IID noise
                    if self.case == 1:
                        obs[sensor_i] = obs[sensor_i] + random.gauss(2, 2)
                        is_random = 1
                    ...
                    # Add calibration failure noise
                        ...
                        obs[sensor_i] = obs[sensor_i] * 10
                        is_random = 1
                    # Add sensor drift noise
                    elif self.case == 4:
                        # obs[sensor_i] = obs[sensor_i] + self._clock / 20
                        obs[sensor_i] = obs[sensor_i] + self._clock / 200
                        is_random = 1
