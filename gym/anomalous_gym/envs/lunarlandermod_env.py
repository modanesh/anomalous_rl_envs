...
class ModLunarLanderEnv(gym.Env, EzPickle):
    metadata = {
    ...
    def __init__(self, case, when_anomaly_starts=40):
        ...
        if self._clock > self.when_anomaly_starts:
            for sensor_i, sensor in enumerate(self.selected_sensors):
                if sensor:
                    # Add IID noise
                    if self.case == 1:
                        obs[sensor_i] = obs[sensor_i] + random.gauss(0, 1)
                        is_random = 1
                    ...
                    # Add sensor drift noise
                        ...
                        # obs[sensor_i] = obs[sensor_i] + self._clock / 1000
                        obs[sensor_i] = obs[sensor_i] + self._clock / 10000
                        is_random = 1
...
class LunarLanderContinuous(ModLunarLanderEnv):
    continuous = True
